...
# 状态定义
STATE_WAITING_CMD = 'WAITING_CMD'      # 初始/停止状态
STATE_STARTED = 'STARTED'              # 已收到 start，等待 resultOK 触发检测
STATE_DETECTING_ONCE = 'DETECTING_ONCE'  # 正在执行单次检测（短暂状态）
                ...
                if state == STATE_WAITING_CMD:
                    if data.lower() == 'start':
                        ...
                        try:
                            ...
                            print(f"❌ 发送 readyOK 失败: {e}")
                        print("🟢 收到 'start'，进入 STARTED 状态，等待 resultOK")

                elif state == STATE_STARTED:
                    if data.lower() == 'resultok' or data.lower() == 'resultokresultok':
                        with client_lock:
                            current_state = STATE_DETECTING_ONCE
                        print("🟢 收到 'resultOK'，触发单次检测")

                # STATE_DETECTING_ONCE 是瞬时状态，不在这里处理指令
        except (ConnectionResetError, ConnectionAbortedError, OSError):
            pass
        finally:
            with client_lock:
                client_socket = None
                current_state = STATE_WAITING_CMD  # 显式重置状态
            print("❌ 客户端已断开，等待新连接...")
    ...
    while True:
        ...
        if state == STATE_DETECTING_ONCE:
            ...
            for box in boxes:
                ...
                x_min = x_center
                y_min = y_center
                if conf > 0.8 and cls_id in TARGET_SET and 200 < x_min < 480 and 170<y_min < 340:
                    # 使用 xyxy 获取精确边界框中心
            ...
            if detected:
                ...
                try:
                    ...
                    print(f"❌ 发送 NONE 失败: {e}")

            # 检测完成后，回到 STARTED 状态，等待下一次 resultOK
            with client_lock:
                current_state = STATE_STARTED
        ...
        if state == STATE_WAITING_CMD:
            ...
            cv2.putText(display_image, "READY - waiting resultOK", (50, 50),
