...
# 灯亮检测函数
# -----------------------------
def detect_lights_old(image, brightness_threshold=180, min_area=100):
    """检测图像中的亮灯区域"""
    # 转换为灰度图
...
def detect_lights(image, brightness_threshold=200, min_area=100, max_area=5000):
    ...
    # 增加基础的 debug 输出
    print(f"\n[Light Detect] 开始检测 - 亮度阈值: {brightness_threshold}, 面积范围: [{min_area}, {max_area}]")
    ...
    for contour in contours:
        ...
        # 2. 计算并输出详尽的 Debug 信息（中心位置、亮度强度）
        center_x = x + (w // 2)
        center_y = y + (h // 2)
        
        # 提取灰度图中的 ROI 区域来计算真实的亮度统计数据
        ...
        #    print("      | Not Matched, pass!")
        #    continue
        # 3. 截取并导出内部图片方便调试
        if not center_x < 150 and center_y < 100:
            print("       | Not Passed Because X,Y not in correct area")
            continue
        roi_color = image[y:y+h, x:x+w]
        # 为了防止画面中有多个反光点导致图片被覆盖，给文件名加上编号
        save_path = f"./lighttest_{light_count + 1}.png" 
        cv2.imwrite(save_path, roi_color)
        print(f"     | 测试图已导出: {save_path}")
    ...
    while True:
        ...
        if state == STATE_DETECTING_ONCE:
            ...
            for box in boxes:
                ...
                # 过滤低置信度和不在目标区域的目标
                if conf > 0.8 and cls_id in TARGET_SET and 200 < x_center < 480 and 170 < y_center < 340:
