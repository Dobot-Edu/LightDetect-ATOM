...
def light_inspection_control(amr, a_robot, client_socket, vision_result_ok=True):
    ...
    if vision_result_ok:
       ...
       # 右手过渡点
        ...
        atom.movJ(targetJoint=np.deg2rad([54.5826,-11.0416,-53.9137,-34.0268,-48.1980,-34.8087,-32.6747]),
             sacle=1.5,
             arm_type=Arm_type_strucrt.right_arm,
             )
        # 目标点
        atom.movJ(targetJoint=np.deg2rad([27.5785,-20.7988,-39.4565,-29.4293,-53.4498,-22.5923,-14.3652]),
             sacle=1.0,
             arm_type=Arm_type_strucrt.right_arm,
             )
        time.sleep(2)
...
if __name__ == "__main__":
    ...
    while (not flag_exit) and tcp_connected:
        ...
        move_state = amr.amr_move(tag_id=1002, theta=1.0)    # amr移动到灯光检测点
        atom.head_control(np.deg2rad([25,15]), duration=0.5)    # 控制头抬高
        time.sleep(1)
        target_1 = np.array([-0.39, 0.94, -0.57])
        time.sleep(1)
        a_robot.dynamic_lift_approach(target_1)
        atom.head_control(np.deg2rad([-50,-25]), duration=0.5)    # 控制头抬高
        time.sleep(2)
        # 触发相机拍照（使用已建立的TCP连接）
        safe_socket_send(client_socket, "start")
        ...
        if vision_result_ok:
            ...
            target_1 = np.array([-0.54,1.36,-0.9])
            a_robot.dynamic_lift_approach(target_1)
            amr.amr_move(tag_id=1001, theta=0.0)    # amr移动到起始点
