...
def execute_pick_and_place_task(amr, client_socket_vision, pick_station, place_station):
    ...
    amr.amr_move(tag_id=pick_station, theta=0.0)    # amr移动到取料点
    time.sleep(0.5)
    ...
    # =========================== 放料流程 ========================================
    # 3.移动到放料点
    amr.amr_move(tag_id=place_station, theta=0.0)    # amr移动到放置点
    ...
    # 5.前往放置点
    plan_info = {
        "targrt": np.deg2rad([5.4576,-1.2115,-8.0403,-9.1641,12.7516,-5.5835,8.0306]),
        "vel": joint_vel,
        "acc": joint_acc,
        "CP": 0 * 0.01,
    }
    ...
    plan_info = {
        "targrt": np.deg2rad([1.39,3.72,7.21,2.84,-9.28,-20.27,-9.78]),
        "vel": joint_vel,
        "acc": joint_acc,
        "CP": 0 * 0.01,
    }
...
if __name__ == "__main__":
    ...
    client_socket_vision_vision = setup_tcp_connection('127.0.0.1', 65432) #建立TCP连接
    if not client_socket_vision_vision: #如果TCP连接失败
        ...
        if (js is not None and js.button_R2_ == True and js.button_X_ == True):     #判断RT + X按钮是否被按下
            ...
            while (not flag_exit) and tcp_connected_vision and tcp_connected_robot:
                ...
                if recognized_text == "Function timeout":
                    ...
                    api_key = "app-kOUa6CXBugof6u48g9Bcr5EG"
                    lang = "ch"
